...
| M4 Browser Fetchers | Dynamic / Stealth / browser sidecar | wait / wait selector / `wait_selector_state` / `blocked_domains` 目标域拦截 / 声明式 `page_action` / stealth 配置入口可工作；真实子资源拦截与 CDP 接管待继续补齐 | `rebar3 eunit -m scrapling_browser_contract_tests`；`rebar3 eunit -m scrapling_dynamic_fetcher_tests`；`rebar3 eunit -m scrapling_stealth_fetcher_tests` | doing |
| M5 Spider Runtime | Request、Scheduler、SessionManager、Engine、Spider、checkpoint、stream | 最小串行 crawl、checkpoint、外部 pause / resume / stream / blocked retry 已就位；后续继续收敛更高阶 runtime parity / export / 观测口径 | `rebar3 eunit -m scrapling_request_tests`；`rebar3 eunit -m scrapling_scheduler_tests`；`rebar3 eunit -m scrapling_session_manager_tests`；`rebar3 eunit -m scrapling_spider_tests`；`rebar3 eunit -m scrapling_checkpoint_tests`；`rebar3 eunit -m scrapling_spider_e2e_tests` | doing |
...
| REQ-0001-005 | `v1-fetchers-browser` | `rebar3 eunit -m scrapling_browser_contract_tests`；`rebar3 eunit -m scrapling_dynamic_fetcher_tests`；`rebar3 eunit -m scrapling_dynamic_session_tests` | 当前证据：Port sidecar 契约、最小 dynamic fetcher、dynamic session 默认参数复用、本地 sidecar E2E 已通过；`cdp_url` 非法/未支持错误、`wait_selector_state` 四态语义、`blocked_domains` exact/subdomain 目标域拦截，以及声明式 `page_action` click DOM 注入已固化 | doing |
...
| REQ-0001-009 | `v1-spider-runtime` | `rebar3 eunit -m scrapling_checkpoint_tests`；`rebar3 eunit -m scrapling_spider_e2e_tests` | 当前证据：checkpoint save/load/cleanup、pause-after-requests -> checkpoint、resume-from-checkpoint、stream item-by-item + live stats、blocked retry recover/exhaust、external pause signal checkpoint/graceful-stop E2E 已通过 | done |
...
| REQ-0001-012 | `v1-repo-bootstrap` / `v1-cli-ai-docs` | `rebar3 eunit` | 当前证据：当前全量 `eunit` 共 `72` 条测试通过；最终全仓门禁待继续扩充 | doing |
