...
| M5 Spider Runtime | Request、Scheduler、SessionManager、Engine、Spider、checkpoint、stream | `Request` 值对象与指纹已就位；后续补齐 crawl / pause / resume / stats / blocked retry / export 路径闭环 | `rebar3 eunit -m scrapling_request_tests`；`rebar3 eunit -m scrapling_spider_tests`；`rebar3 eunit -m scrapling_spider_e2e_tests` | doing |
...
| REQ-0001-008 | `v1-spider-runtime` | `rebar3 eunit -m scrapling_request_tests`；`rebar3 eunit -m scrapling_spider_tests` | 当前证据：`scrapling_request` 值对象、复制语义、domain 与最小 fingerprint 已通过；scheduler / engine / spider 待继续补齐 | doing |
...
| REQ-0001-012 | `v1-repo-bootstrap` / `v1-cli-ai-docs` | `rebar3 eunit` | 当前证据：当前全量 `eunit` 共 `50` 条测试通过；最终全仓门禁待继续扩充 | doing |
