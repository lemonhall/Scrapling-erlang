...
| M5 Spider Runtime | Request、Scheduler、SessionManager、Engine、Spider、checkpoint、stream | `Request` / `Scheduler` / `SessionManager` 骨架已就位；后续补齐 crawl / pause / resume / stats / blocked retry / export 路径闭环 | `rebar3 eunit -m scrapling_request_tests`；`rebar3 eunit -m scrapling_scheduler_tests`；`rebar3 eunit -m scrapling_session_manager_tests`；`rebar3 eunit -m scrapling_spider_tests`；`rebar3 eunit -m scrapling_spider_e2e_tests` | doing |
...
| REQ-0001-008 | `v1-spider-runtime` | `rebar3 eunit -m scrapling_request_tests`；`rebar3 eunit -m scrapling_scheduler_tests`；`rebar3 eunit -m scrapling_session_manager_tests`；`rebar3 eunit -m scrapling_spider_tests` | 当前证据：`scrapling_request`、`scrapling_scheduler`、`scrapling_session_manager` 的最小契约已通过；engine / spider 待继续补齐 | doing |
...
| REQ-0001-012 | `v1-repo-bootstrap` / `v1-cli-ai-docs` | `rebar3 eunit` | 当前证据：当前全量 `eunit` 共 `57` 条测试通过；最终全仓门禁待继续扩充 | doing |
