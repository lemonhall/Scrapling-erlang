...
spider_external_pause_saves_checkpoint_test() ->
    TestPid = self(),
    GateRef = make_ref(),
    Checkpoint = scrapling_checkpoint:new(temp_crawldir()),
    Manager0 = scrapling_session_manager:new(),
    Fetcher = fun(Request) -> fetch_pause_signal_page(TestPid, GateRef, Request) end,
    {ok, Manager1} = scrapling_session_manager:add(<<"default">>, scrapling_session_manager:custom(Fetcher), Manager0),
    try
        {ok, Runner} = scrapling_spider:run(scrapling_test_spider_minimal, #{session_manager => Manager1, checkpoint_manager => Checkpoint}),
        WorkerPid = receive
            {pause_list_fetch_started, FetchPid, GateRef0} when GateRef0 =:= GateRef ->
                FetchPid
        after 2000 ->
            erlang:error(pause_list_fetch_gate_timeout)
        end,
        ok = scrapling_spider:pause(Runner),
        WorkerPid ! {allow_pause_list_fetch, GateRef},
        Result = scrapling_spider:await(Runner, 2000),
        Stats = scrapling_crawl_result:stats(Result),
        ?assertEqual(false, scrapling_crawl_result:completed(Result)),
        ?assertEqual([#{kind => <<"list">>, title => <<"List Page">>}], scrapling_crawl_result:items(Result)),
...
spider_external_pause_without_checkpoint_stops_gracefully_test() ->
    TestPid = self(),
    GateRef = make_ref(),
    Manager0 = scrapling_session_manager:new(),
    Fetcher = fun(Request) -> fetch_pause_signal_page(TestPid, GateRef, Request) end,
    {ok, Manager1} = scrapling_session_manager:add(<<"default">>, scrapling_session_manager:custom(Fetcher), Manager0),
    {ok, Runner} = scrapling_spider:run(scrapling_test_spider_minimal, #{session_manager => Manager1}),
    WorkerPid = receive
        {pause_list_fetch_started, FetchPid, GateRef0} when GateRef0 =:= GateRef ->
            FetchPid
    after 2000 ->
...
fetch_pause_signal_page(TestPid, GateRef, Request) ->
    Url = scrapling_request:url(Request),
    case Url of
        <<"https://example.com/list">> ->
            TestPid ! {pause_list_fetch_started, self(), GateRef},
            receive
                {allow_pause_list_fetch, GateRef} -> ok
            after 2000 ->
                erlang:error(pause_list_fetch_release_timeout)
            end,
            scrapling_response:new(200, <<"OK">>, [{"content-type", "text/html; charset=utf-8"}], <<"<html><body><h1>List Page</h1><a class='next' href='https://example.com/detail'>Next</a></body></html>">>, Url, get, [], #{});
        <<"https://example.com/detail">> ->
            scrapling_response:new(200, <<"OK">>, [{"content-type", "text/html; charset=utf-8"}], <<"<html><body><h1>Detail Page</h1></body></html>">>, Url, get, [], #{});
